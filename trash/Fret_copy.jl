...
mutable struct FretContext
    ...
    function FretContext(system::System, environment::Environment, simulation_details::SimulationDetails)
        ...
        g       = zeros(ComplexF64, (n_sys, n_sys, n_itr))
        g′      = zeros(ComplexF64, (n_sys, n_sys, n_itr))
        g″      = zeros(ComplexF64, (n_sys, n_sys, n_itr))
        G       = zeros(ComplexF64, (n_sys, n_sys, n_itr)) 
...
function calc__g_with_threads!(context::FretContext)
    ...
    g_locals = [zeros(ComplexF64, n_sys, n_sys, n_itr) for _ in 1:n_ths]
    ...
    @inbounds @threads for osc_idx in 1:n_osc
        ...
        @inbounds for b in 1:n_sys, a in b:n_sys
            ...
            @inbounds for time_idx = 1:n_itr
                ...
                g_local[a,b,time_idx] += common * ( (coth * (1.0 - cos(ωt))) + 1.0im*(sin(ωt) - ωt) )
            end
        end
...
function calc__rates!(context::FretContext)
    ...
    @inbounds for b in 1:n_sys, a in (b+1):n_sys
        ...
        @inbounds for time_idx in 1:n_itr
            ...
            G[a,b,time_idx] = exp(real(exponent_ab)) * (cos(imag(exponent_ab)) + 1.0im*sin(imag(exponent_ab)))
            G[b,a,time_idx] = exp(real(exponent_ba)) * (cos(imag(exponent_ba)) + 1.0im*sin(imag(exponent_ba)))
            ...
            rate[a,b] += trapezoidal_weight * real(G[a,b,time_idx])
            rate[b,a] += trapezoidal_weight * real(G[b,a,time_idx])
        end
